...
# ── ADDED: Native language system prompts ─────────────────────────────────────
# Applied uniformly across all 5 languages so the comparison stays fair.
# Each says the same thing in the target language: you are a math assistant,
# answer with a single number only, never explain.
# Placed in the system turn (separate from the user question) so the model
# treats it as a persistent constraint rather than part of the question.
# ──────────────────────────────────────────────────────────────────────────────
LANG_TO_SYSTEM_PROMPT = {
    "en": "You are a math assistant. Always respond with a single number only. Never explain your answer.",
    "bn": "আপনি একজন গণিত সহকারী। সর্বদা শুধুমাত্র একটি সংখ্যা দিয়ে উত্তর দিন। কখনো ব্যাখ্যা করবেন না।",
    "sw": "Wewe ni msaidizi wa hisabati. Daima jibu kwa nambari moja tu. Usitoe maelezo yoyote.",
    "te": "మీరు గణిత సహాయకుడు. ఎల్లప్పుడూ కేవలం ఒక్క సంఖ్యతో మాత్రమే సమాధానం ఇవ్వండి. ఎప్పుడూ వివరణ ఇవ్వకండి.",
    "zh": "你是一个数学助手。始终只用一个数字回答。永远不要解释你的答案。",
}


LANG_TO_INSTRUCTIONS = {
    "en": """Answer the following math question with a single number only. No explanation. Do not repeat the question. Do not write any explanation after the answer.
...
Answer:""",
    ...
    "bn": """নিচের গণিত প্রশ্নের উত্তর শুধুমাত্র একটি সংখ্যায় দিন। কোনো ব্যাখ্যা নয়। প্রশ্নটি পুনরাবৃত্তি করবেন না। উত্তরের পরে কোনো ব্যাখ্যা লিখবেন না।
...
উত্তর:""",
    ...
    "sw": """Jibu swali hili la hesabu kwa nambari moja tu. Bila maelezo. Usirudie swali. Usiandike maelezo yoyote baada ya jibu.
...
Jibu:""",
    ...
    "te": """క్రింది గణిత ప్రశ్నకు కేవలం ఒక్క సంఖ్యలో సమాధానం ఇవ్వండి. వివరణ అవసరం లేదు. ప్రశ్నను పునరావృతం చేయవద్దు. సమాధానం తర్వాత ఎటువంటి వివరణ రాయవద్దు.
...
సమాధానం:""",
    ...
    "zh": """请用一个数字回答以下数学题。不需要解释。不要重复问题。不要在答案后写任何解释。
...
# ── CHANGED: system_prompt added as a parameter ───────────────────────────────
# Passed as a separate system turn so the model treats it as a persistent
# formatting constraint independent of the question content.
# ──────────────────────────────────────────────────────────────────────────────
def call_with_retry(prompt: str, system_prompt: str, model=model_name, max_retries=5) -> str:
...
def fetch_single_run(run_idx: int, prompt: str, system_prompt: str, expected: str, q_label: str, lang: str):
    try:
        raw = call_with_retry(prompt, system_prompt)
...
def run_inference_for_lang(lang: str):
    ...
    system_prompt = LANG_TO_SYSTEM_PROMPT[lang]  # fetched once per language
    ...
    for idx, row in df.iterrows():
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            future_to_run = {
                executor.submit(
                    fetch_single_run, run, prompt, system_prompt, str(row["answer"]), q_label, lang
                ): run
                for run in range(N_RUNS)
            }
