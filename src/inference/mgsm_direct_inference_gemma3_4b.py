...
def call_model(prompt: str, max_new_tokens: int = 32) -> str:
    """
    Run a single greedy inference pass with Gemma-3-4B-IT using
    the chat template expected by the instruct variant.
    """
...
def run_inference_for_lang(lang: str):
    ...
    # df = df.head(10)  # uncomment to run a quick smoke-test
